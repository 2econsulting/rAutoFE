...
autoFE <- function(train, valid, test, x, y, savePath, verbose=FALSE){
  ...
  train <- reduceDimCat_transform(data = train, fit = fit_removeOneLevel)
  valid <- reduceDimCat_transform(data = valid, fit = fit_removeOneLevel)
  test  <- reduceDimCat_transform(data = test,  fit = fit_removeOneLevel)
